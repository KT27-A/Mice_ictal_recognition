...
if __name__=="__main__":
    ...
    val_dataloader   = DataLoader(val_data, batch_size = opts.batch_size, shuffle=True, num_workers = opts.n_workers, pin_memory = True, drop_last=False)
    ...
    for epoch in range(begin_epoch, opts.n_epochs + 1):
        ...
        if opts.val_file_2:
            ...
            val_dataloader_2 = DataLoader(val_data_2, batch_size = opts.batch_size, 
            shuffle=True, num_workers = opts.n_workers, pin_memory = True, drop_last=False)
            print("Length of validation_2 data = ", len(val_data_2))
            with torch.no_grad():
                for i, (inputs, targets) in enumerate(val_dataloader_2):
